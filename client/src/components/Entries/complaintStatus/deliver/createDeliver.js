...
export default class CreateDeliver extends Component {
    constructor(props){
        super(props);
        this.state = {
            delivertransactionid: '',
            deliverdate: '',
            delivercomplaintcaseid: '',
			deliverremarks: '',
			RFSid: '',
    ...
    componentDidMount(){
		...
		let randomnumber = 'DPC-' + number[1];
        this.setState({ delivertransactionid: randomnumber });
         ...
         axios
			...
			.then((response) => {
				...
				this.setState({
                        delivercomplaintcaseid: response.data.caseid,
                        complaint: response.data,
    /* --------------------------------------------- Complaint Details --------------------------------------------- */

                        complaintcaseid: response.data.caseid,
                        complaintdate: response.data.complaintdate,
                        customer: Object.entries(response.data.client)[1].slice(1),
                        department: Object.entries(response.data.departmentname)[1].slice(1),
    ...
    /*------------------------------------------------------- Update Complaint --------------------------------------------------------------------------- */				
						...
						axios
						.get('http://localhost:4000/api/complaintstatus/recievedfromservice/')
						.then((recieve) => {

							let recieveObj;

							recieve.data.map(r1 => {
							let tempreceivedcaseid = [r1.receivedcaseid];
							tempreceivedcaseid.map(r2 => {
								let tempsenttocaseid = [r2.senttocaseid];
								tempsenttocaseid.map(r3 => {
									let tempcomplaintno = [r3.complaintno];
									tempcomplaintno.map(r4 => {
										let tempfeedback = [r4.feedback];
										tempfeedback.map(r5 => {
											if(this.props.match.params.complaintid === r5._id){
												recieveObj = {
													Rfeedbackdate: r1.receiveddate,
													Rfeedback: r1.recievedremarks,
													Rfeedbackstatus: 'Recieved from Service',
												}
												ArrFeedbackRecieve.push(recieveObj);
												console.log(ArrFeedbackRecieve)
											}
										})
									})
								})
    ...
    onSubmit() {
		const submitObj = {
			delivertransactionnumber: this.state.delivertransactionid,
			deliverdate: this.state.deliverdate,
			receivedid: this.state.RFSid,
            deliverremarks: this.state.deliverremarks
		};

		console.log(submitObj);

		axios
			.post('http://localhost:4000/api/complaintstatus/deliver/add', submitObj)
  ...
  render() {
	...
	if(!isEmpty(this.state.complaintdate)){
		var datecomplaintdate = new Date(this.state.complaintdate);
    ...
    if (!isEmpty(this.state.addproducts)) {
		...
		if (!isEmpty(this.state.BringInWarranty)) {
			BringInWarranty1 = this.state.BringInWarranty.map((item) => {
				...
				BringInWarranty1 = this.state.BringInWarranty.map((item1) => {

				if(!isEmpty(item.oemwarrantyfrom)){
				var dateOemwarrantyfrom = new Date(item1.oemwarrantyfrom);
				var dateOemwarrantyto = new Date(item1.oemwarrantyto);
				...
				return (
					<Fragment>
						<tr>
							<td>{item1.serialno}</td>
							<td>Warranty</td>
							<td>OEM</td>
							<td>{item1.serviceprovider}</td>
    ...
    return (
      <div>
        <h2 className="text-center">Deliver Products To Customer :</h2>
				...
				<form style={{ marginTop: '5%' }} onMouseMove={this.getrecievedfromservicingid}>
						...
						<div className="col-6">
							<div className="form-group">
								...
								<input
									...
									placeholder=""
									name="delivertransactionid"
									value={this.state.delivertransactionid}
					...
					<div className="row">
						<div className="col-6">
							...
							<input
								type="text"
								className="form-control"
								name="delivercomplaintcaseid"
								placeholder="Challan No"
								value={this.state.delivercomplaintcaseid}
								onChange={this.onChange}
							/>
						</div>
                        <div className="col-6">
					...
					<div className="row">
						&nbsp;&nbsp;<input
							...
							disabled={!(this.state.deliverdate && this.state.deliverremarks)}
				...
				<div className="row">
                  <div className="col-6">
                      <div className="form-group">
                    <label style = {lblStyle}>Complaint Case Id :</label>&nbsp;&nbsp;
                    <label>{this.state.complaintcaseid}</label>
                    </div>
                  </div>
                   <div className="col-6">
                       <div className="form-group">
                    <label style = {lblStyle}>Complaint Date :</label>&nbsp;&nbsp;
                    <label>{complaintDate}</label>
                    </div>
                  </div>
                </div>
                <div className="row">
            ...
            <div>
                <ul className="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <div className="tab-pane fade" id="additionalitems" role="tabpanel" aria-labelledby="profile-tab">
							<table id="tab" className="table table-bordered mb-4">
								<thead style={{ backgroundColor: '#d8d8d8' }}>
									<tr>
										<td>Item Name</td>
