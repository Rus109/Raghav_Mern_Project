...
class EditProdReg extends Component {
    ...
    componentDidMount() {
      ...
      axios
      .get(
        "http://localhost:4000/api/proreg/" + this.props.match.params.id
      )
      .then(response => {
        var dataProregDate = new Date(response.data.date);
        var dataDataProregDate = dataProregDate.toISOString().substr(0, 10);

        this.setState({
          refno1: response.data.refno1,
          refno2: response.data.refno2,
          prodregdate: dataDataProregDate,
          customerid: Object.entries(response.data.customer)[0].slice(1),
          customertypeid: Object.entries(response.data.customertype)[0].slice(1),
          departmentid: Object.entries(response.data.department)[0].slice(1),
    ...
    axios
      .post("http://localhost:4000/api/proreg/" + this.props.match.params.id, obj)
      .then(res => console.log(res.data));
    ...
    onEnterData(e){
      ...
      axios
      .post("http://localhost:4000/api/proreg/product/" + this.props.match.params.id, obj2)
      .then(res => console.log(res.data));
     
      axios
      .get("http://localhost:4000/api/proreg/" + this.props.match.params.id)
      .then(response => {
        this.setState({ 
          productMapData: response.data.products
        });
    ...
    render() {
      var proregid = this.props.match.params.id;
        ...
        const Customer = this.state.customer.map((cus, i) => (
          ...
          const tabList =  this.state.productMapData.map(function(object, i){
            return <UpdateTableList obj={object} ids = {proregid} key={i} ></UpdateTableList>
          })
        ...
        return (
          ...
          <Link to="/api/productregistration" className="btn btn-info" >
              Back to List
                  ...
                  <div className="form-group">
                      ...
                      <input type="button" value="Update Form" className="btn btn-primary" onClick = {this.onUpdateForm} />
                  </div>
                  </form>
          </div>
          ...
          <input type="button" onClick = {this.onSubmit} value="Save" className="btn btn-info" />
          </div>
         );
    }
}
 
export default EditProdReg;