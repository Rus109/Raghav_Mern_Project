...
class UpdateSubTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      product: [],
  ...
  componentDidMount() {
    ...
    if (this.props.dat.oemwarranentyfrom !== null) {
      ...
      var dataDateOemWarranentyFrom = dateOemWarranentyFrom
        .toISOString()
        .substr(0, 10);
  ...
  onSubTableSubmit() {
    ...
    axios
      .post(
        "http://localhost:4000/api/proreg/product/" +
          id +
          "/" +
          this.props.dat.productsId,
        obj
      )
